...
nssbits() {
	...
	case ${t} in
	                                     x32) echo USE_X32=1;;
	amd64|amd64_fbsd|n64|arm64|s390x|sparc64) echo USE_64=1;;
	                                       *) ;;
	esac
}
	...
	if multilib_is_native_abi; then
		...
		if use utils; then
			nssutils="addbuiltin atob baddbdir btoa certcgi certutil checkcert
				...
				multinit nonspr10 ocspclnt oidcalc p7content p7env p7sign p7verify
				pk11mode pk12util pp rsaperf selfserv shlibsign signtool signver
				ssltap strsclnt symkeyutil tstclnt vfychain vfyserv"
		fi
...
pkg_postinst() {
	multilib_foreach_abi nsspostinst
}
...
pkg_postrm() {
	multilib_foreach_abi nsspostrm
}
